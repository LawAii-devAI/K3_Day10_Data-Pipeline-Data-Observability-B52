...
def main() -> None:
    ...
    if not settings.paths.baseline_metrics.exists():
        raise RuntimeError("Baseline metrics not found. Run script/run_phase1.py first.")
    ...
    print(f"Corrupted: {corrupted_bundle.summary}")
    print(f"Repaired: {repaired_bundle.summary}")
